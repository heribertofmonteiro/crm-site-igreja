...
<div class="card card-primary">
    ...
    <form action="{{ route('admin.worship.rehearsals.store') }}" method="POST">
        @csrf
        <div class="card-body">
            <div class="form-group">
                ...
                <select name="worship_team_id" id="worship_team_id" class="form-control select2" required>
                    ...
                    @foreach($teams as $team)
                        <option value="{{ $team->id }}" {{ old('worship_team_id') == $team->id ? 'selected' : '' }}>
                            {{ $team->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input type="datetime-local" name="scheduled_at" id="scheduled_at" class="form-control @error('scheduled_at') is-invalid @enderror" value="{{ old('scheduled_at') }}" required>
                    @error('scheduled_at') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>
                <div class="col-md-6 form-group">
                    <label for="location">Local</label>
                    <input type="text" name="location" id="location" class="form-control" value="{{ old('location') }}" placeholder="Ex: Templo Principal">
                </div>
            </div>
        ...
        <div class="card-footer">
            <button type="submit" class="btn btn-primary">Agendar Ensaio</button>
